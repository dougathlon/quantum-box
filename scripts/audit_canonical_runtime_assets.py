...
RUNTIME_MANIFEST = PACKAGE / "manifests/runtime-handoff.json"
EXPECTED_MANIFEST_HASHES = {
    SOURCE_MANIFEST: "aa12069a34edd3b60585b476bac6f73d01dfd503fbbbac6f9808b7e30ebdb842",
    RUNTIME_MANIFEST: "1ffdb24e076cb10c51402523c7b66ef462a8a8ba9a492d29bc1560d32b43e9ae",
}
ALLOWED_PIXELS = {
    (43, 28, 20, 255),
    (86, 67, 48, 255),
...
def main() -> None:
    ...
    require(runtime["schemaVersion"] == "quantum-box-canonical-runtime-handoff-v2", "runtime schema drift")
    require(runtime["status"] == "immutable-canonical-handoff", "runtime approval drift")
    ...
    require(len(runtime_entries) == 45, "canonical runtime file count drift")
    ...
    actual_paths = {
        str(path.relative_to(PACKAGE)) for path in (PACKAGE / "assets").rglob("*.png")
    }
    require(actual_paths == manifested_paths, "runtime manifest coverage drift")
    ...
    for file_id in (
        ...
        entry = files_by_id[file_id]
        require(entry["kind"] == "runtime-morph-strip", f"morph kind drift: {file_id}")
        require(
            [frame["frameId"] for frame in entry["frames"]]
            == [f"morph-{index}" for index in range(7)],
            f"true-morph frame order drift: {file_id}",
        )

    ski_rule = runtime["skipixlDesignerEncounter"]
    require(ski_rule["morphAsset"] is None, "SkiPixl must not gain a morph asset")
    require(ski_rule["behavior"] == "wizard-waits-at-lodge", "SkiPixl lodge rule drift")
    require(not list((PACKAGE / "assets").rglob("*skipixl*morph*.png")), "unexpected SkiPixl morph")
        ...
        json.dumps(
            {
                ...
                "runtimeHandoffSha256": digest(RUNTIME_MANIFEST),
